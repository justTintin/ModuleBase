...
public class DataBaseHandler extends Handler
{
    ...
    private <T extends BaseModel> void insert(Message msg)
    {
        ...
        if (!TextUtils.isEmpty(tableName)
                && msgObj.contentConditionList != null
                && msgObj.contentConditionList.size() > 0)
        {
            ...
            if (database != null)
            {
                ...
                try
                {
                    ...
                    {
                        if (condition != null
                                && condition.contentValues != null)
                        {
                            long id = database.insert(tableName,
                                    null,
                                    condition.contentValues);//进行插入操作
                            if (id != -1)
                            {//插入成功，将对应的model 值放入到 successModels 中
                ...
                finally
                {
                    ...
                    postToMainLoop(msgObj.listener,
                            DatabaseOptionType.OPTION_INSERT,
                            msgObj.claz,
                            successModels,
                            failModels);//失败回调到主线程
                }
                ...
                && msgObj.contentConditionList.size() > 0)
        {
            ...
            if (database != null)
            {
                ...
                try
                {
                    for (ContentCondition<T> condition : msgObj.contentConditionList)
                    {
                        if (condition != null
                                && condition.contentValues != null)
                        {
                            int count = database.update(tableName,
                ...
                finally
                {
                    database.endTransaction();
                    postToMainLoop(msgObj.listener,
                            DatabaseOptionType.OPTION_UPDATE,
                            msgObj.claz,
                            successModels,
                            failModels);
                }
                ...
                && msgObj.contentConditionList.size() > 0)
        {
            ...
            if (database != null)
            {
                ...
                try
                {
                    for (ContentCondition<T> condition : msgObj.contentConditionList)
                    {
                        if (condition != null
                                && condition.contentValues != null)
                        {
                            long id = database.replace(tableName,
                                    null,
                                    condition.contentValues);
                            if (id != -1)
                            {
                ...
                finally
                {
                    database.endTransaction();
                    postToMainLoop(msgObj.listener,
                            DatabaseOptionType.OPTION_REPLACE,
                            msgObj.claz,
                            successModels,
                            failModels);
                }
                    ...
                    if (msgObj.deleteListener != null)
                    {
                        ...
                        uiHandler.post(new Runnable()
                        {
                            ...
                            public void run()
                            {
                                ...
                                msgObj.deleteListener.onDeleteCallback(msgObj.claz,
                                        tempRows);
                            }
                        });
                    }
    ...
    private <T extends BaseModel> void postToMainLoop(
            final DBOperateAsyncListener listener,
            final DatabaseOptionType type, final Class<T> claz,
            final List<T> successModels, final List<T> failModels)
    {
        if (listener != null)
        {
            uiHandler.post(new Runnable()
